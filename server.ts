...
async function startServer() {
  ...
  app.post('/api/tutor', async (req, res) => {
    try {
      ...
      if (!gkey) {
        return res.status(500).json({ reply: "Lỗi hệ thống: Chưa cấu hình Groq API Key. Vui lòng nhập key trong phần Cài đặt." });
      }
...
- Để tham gia học miễn phí: Nhấp chuột vào nút "Đăng ký ngay" hoặc "Bắt đầu học" ở trang chủ.

QUY TẮC PHẢN HỒI BẮT BUỘC 100%:
1. TRẢ LỜI CỰC KỲ CÔ ĐỌNG, ĐÚNG TRỌNG TÂM: Chỉ trả lời trong vòng tối đa 2-3 câu ngắn. Tránh viết nhiều, dài dòng, lặp từ, lê thê, hay chào hỏi rườm rà.
...
`;
      ...
      if (isMissingKey) {
        res.json({ reply: "⚠️ Groq API Key không hợp lệ hoặc chưa được cấu hình! Vào phần Cài đặt để nhập key miễn phí từ console.groq.com 🚀" });
      } else {
...
`;
  ...
  app.post('/api/grade', async (req, res) => {
    try {
      ...
      const gkey = getGroqKey(req.headers['x-api-key'] as string);
      if (!gkey) {
        return res.status(500).json({ score: 0, feedback: "Lỗi hệ thống: Chưa cấu hình Groq API Key." });
      }
