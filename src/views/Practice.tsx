...
export function Practice() {
  ...
  useEffect(() => {
    const fetchPractices = async () => {
      try {
        ...
        try { handleFirestoreError(e, OperationType.GET, 'practices'); } catch {}
      } finally {
        setLoading(false);
      }
    };
  ...
  const getLevelIcon = (level: string) => {
    switch(level) {
      case 'Đồng': return <Shield className="w-5 h-5" />;
      case 'Bạc': return <Star className="w-5 h-5" />;
      case 'Vàng': return <Award className="w-5 h-5" />;
      case 'Kim Cương': return <Diamond className="w-5 h-5" />;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-1">Kho Bài tập & Dự án</h1>
      <p className="text-sm text-slate-500 mb-8">Thực hành tư duy logic với các dự án thực tế. Code đúng sẽ nhận thêm điểm.</p>

      {loading ? (
        <div className="p-12 text-center text-slate-500 text-sm">Đang tải...</div>
      ) : !activeProject ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {projects.map(p => (
            <div 
              key={p.id} 
              className="card p-5 flex flex-col items-start cursor-pointer hover:border-[#0052cc] hover:shadow-md transition-all" 
              onClick={() => { audioService.playClick(); setActiveProject(p); }}
            >
              <div className={`p-2 rounded-lg mb-3 ${getLevelColor(p.level)}`}>
                {getLevelIcon(p.level)}
              </div>
              <h3 className="font-bold text-base text-slate-900 dark:text-white mb-2">{p.title}</h3>
              <div className="text-sm text-slate-500 flex-1 line-clamp-3 mb-4 prose prose-sm">
                <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]}>{p.description}</ReactMarkdown>
              </div>
              <div className="mt-auto flex w-full items-center justify-between text-sm pt-4 border-t border-gray-100 dark:border-slate-700">
                <span className="font-medium text-[#0052cc]">+{p.points} Điểm</span>
        ...
        <div className="card p-6 md:p-8">
          <button onClick={() => { audioService.playClick(); setActiveProject(null); }} 
            className="text-slate-500 hover:text-slate-900 dark:hover:text-white font-medium text-sm mb-5 inline-flex items-center gap-1"
          >
            ← Quay lại danh sách
          </button>
          ...
          <div className="prose prose-blue max-w-none text-slate-700 dark:text-slate-300 mb-6 border border-gray-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 p-5 rounded-xl">
          ...
          <IDE 
            ...
            contextTitle={`Dự án: ${activeProject.title}`}
          />
        </div>
      )}
    </div>
